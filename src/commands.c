...
void do_save(player * p, char *str)
{
  ...
  if (p->residency == NON_RESIDENT)
  {
    pg_log("error", "Tried to save a non-resi, (chris)");
    return;
  }
  save_player(p);
...
{
  (void)p;
  return update_password(entered);
}
...
void set_favorites(player * p, char *str)
{
  ...
  if (*arg_two && !strcasecmp(arg_two, "blank"))
  {
	...
	pg_log("error", "Error in set_favorite");
	return;
    }
  ...
  switch (choice)
  {
	...
	return;
      ...
      break;
    default:
      pg_log("error", "Error in setting a favorite...");
      return;
  }
...
void toggle_email_site_flags(player * p, char *str)
{
  ...
  *opt++ = 0;
  ...
  if (!strcasecmp(opt, "public"))
    ...
    ; /* private is the default (neither pub nor fri) */
...
void newsetpw2(player * p, char *str)
{
  ...
  else
  {
    ...
    if (p->email[0] != 2)
      p->residency &= ~NO_SYNC;
    save_player(p);
    snprintf(p->saved->email, MAX_EMAIL, "%s", p->email);
  }
}
...
int set_players_email(player * p, char *str)
{
  ...
  if (config_flags & cfWELCOMEMAIL && !(p->residency & (PSU | SU | ADC)))
  {
    ...
    {
      pg_log("error", "Need a files/email_welcome.msg !!!");
      return 0;
    }
/*
   sprintf(stack,
 ...
 */
    {
      ...
      if (pid == 0)
      {
        /* child — redirect stdin from welcome file */
        int mailfd = open("files/email_welcome.msg", O_RDONLY);
        if (mailfd >= 0)
        {
          dup2(mailfd, STDIN_FILENO);
          close(mailfd);
        }
        /* build subject line safely without shell interpolation */
        snprintf(stack, STACK_SIZE, "Welcome message to %s@%s",
                 p->name, p->inet_addr);
        execlp("mail", "mail", "-s", stack, p->email, (char *)NULL);
        _exit(127);
      }
      /* parent — don't wait, SIGCHLD will reap */
    }
    stack = oldstack;
