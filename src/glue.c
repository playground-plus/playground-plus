...
void pg_log(char *filename, char *string)
{
  int fd;
  int length;
  char *oldstack = stack;
...
void handle_error(char *error_msg)
{
  ...
  {
    stack = stack_start;
    pg_log("error", "Immediate PANIC shutdown.");
    exit(-1);
  }
  sys_flags |= PANIC;
  ...
  pg_log("error", error_msg);
  pg_log("boot", "Abnormal exit from error handler");

  /* dump possible useful info */

  pg_log("dump", "------------ Starting dump");

  sprintf(stack_start, "Errno set to %d, %s", errno, strerror(errno));
  stack = end_string(stack_start);
  pg_log("dump", stack_start);

  if (current_player)
  {
    pg_log("dump", current_player->name);
    if (current_player->location)
    {
      ...
      pg_log("dump", "No room of current player");
    ...
    stack = end_string(stack_start);
    pg_log("dump", stack_start);
    pg_log("dump", current_player->ibuffer);
  }
  else
    pg_log("dump", "No current player !");
  if (current_room)
  {
...
void sighup(int c)
{
  pg_log("boot", "SIGHUP caught ... being snobbish towards it tho...");
...
void close_down(void)
{
  ...
  if (sys_flags & VERBOSE || sys_flags & PANIC)
    pg_log("boot", "Timer Stopped");

  if (sys_flags & VERBOSE || sys_flags & PANIC)
    pg_log("boot", "Saving all players.");
  for (scan = flatlist_start; scan; scan = scan->flat_next)
    save_player(scan);
  if (sys_flags & VERBOSE || sys_flags & PANIC)
    pg_log("boot", "Syncing to disk.");
  sync_all();
  ...
  {
    unlink("junk/PID");
    pg_log("boot", "Program exited normally.");
    exit(0);
  }
}
...
int main(int argc, char *argv[])
{
  ...
  {
    pg_log("boot", "Check your port configuration, setting to default 7777");
    port = 7777;
  }
  active_port = port;
  boot(port);
  ...
  pg_log("boot", "Ready and listening for connections");	/* Another formatting line ... -grin- */
...
{
  FILE *f;
  char psc[88], red[240], *oldstack = stack, psp[80];
  int my_pid = getpid();
  ...
  snprintf(psc, sizeof(psc), "ps %s", psp);
  if (!(f = popen(psc, "r")))
  {
