...
void sync_news(newsgroup * group, news_header * nh, char *body)
{
  ...
  if (*body)
  {
    ...
    IGNORE_RET(write(fd, body, strlen(body)));
    close(fd);
  }
...
news_header *load_news_header(newsgroup * group, char *fname)
{
  ...
  IGNORE_RET(fread(nh, sizeof(news_header), 1, ni));
  fclose(ni);
...
void save_str_public(char *filename, char *str)
{
  ...
  IGNORE_RET(write(fd, str, strlen(str)));
...
void post_news(player * p, char *str)
{
  ...
  *scan = '\0';
  ...
  /* setup its stuffs */
  snprintf(nh->name, MAX_NAME, "%s", p->name);
  strncpy(nh->header, str, MAX_TITLE - 1);
  nh->header[MAX_TITLE - 1] = '\0';
...
void followup(player * p, char *str)
{
  ...
  if (!nh)
  {
    ...
    pg_log("error", "malloc failed in news followup");
    return;
  }
  memset(nh, 0, sizeof(news_header));
  ...
  {
    snprintf(nh->header, MAX_TITLE, "%s", scan->header);
  }
  else
  {
...
void news_stats(player * p, char *str)
{
  ...
  stack += sprintf(stack, " The NewsGroups array is %zu bytes in size.\n",
		   sizeof(NewsGroups));

  for (i = 0; NewsGroups[i].name[0]; i++)
  {
    curcnt = count_news_articles(&NewsGroups[i]);
    stack += sprintf(stack, " Newsgroup %-20s - %6d posting/s, %zu bytes\n",
