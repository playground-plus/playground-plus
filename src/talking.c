...
void process_review(player * p, char *message, int length)
{
  ...
  {
    strncpy(p->rev[i].review, p->rev[i + 1].review, MAX_REVIEW - 1);
    p->rev[i].review[MAX_REVIEW - 1] = '\0';
    for (j = 0; j < MAX_REVIEW; j++)
      p->rev[i + 1].review[j] = 0;
  }
  strncpy(p->rev[i].review, message, length);
  p->rev[i].review[length < MAX_REVIEW ? length : MAX_REVIEW - 1] = '\0';
}
...
{
  char *oldstack, *msg, *text, *pstring, *mid, *s;
  player **list, *scan;
  int n;
