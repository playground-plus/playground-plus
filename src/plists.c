...
void login_upgrade_pw1(player *, char *), login_upgrade_pw2(player *, char *),
  finish_login(player *);
...
/* generate a random SHA-512 salt */

static void generate_salt(char *salt, int saltlen)
{
  ...
  if (f)
  {
    IGNORE_RET(fread(randbuf, 1, sizeof(randbuf), f));
    fclose(f);
  }
  else
  {
    ...
    srand(time(0) ^ getpid());
    for (i = 0; i < (int)sizeof(randbuf); i++)
      randbuf[i] = rand() & 0xff;
  }
  strcpy(salt, "$6$");
  for (i = 0; i < saltlen && i < (int)sizeof(randbuf); i++)
    salt[i + 3] = chars[randbuf[i] % (sizeof(chars) - 1)];
  salt[i + 3] = '$';
  salt[i + 4] = '\0';
}

/* crypts the password using SHA-512 */

char *update_password(char *oldpass)
{
  static char salt[24];
  generate_salt(salt, 16);
  return crypt(oldpass, salt);
}
...
void extract_player(char *where, int length)
{
  ...
  if (!old)
  {
    ...
    sp->lower_name[MAX_NAME - 1] = '\0';
    strncpy(player_loading, sp->lower_name, MAX_NAME - 1);
    player_loading[MAX_NAME - 1] = '\0';
    sp->rooms = 0;
    sp->mail_sent = 0;
    sp->mail_received = 0;
    sp->list_top = 0;
...
void hard_load_files(void)
{
  ...
  pg_log("boot", "Loading all player files");
...
void sync_all()
{
  ...
  for (c = 'a'; c <= 'z'; c++)
    sync_to_file(c, 0);
  pg_log("sync", "Full Sync Completed.");
}
...
int remove_player_file(char *name)
{
  ...
  for (c = stack; *c; c++)
  {
    ...
    {
      pg_log("error", "Remove bad name from save files");
      return 0;
    }
  }
...
void save_player(player * p)
{
  ...
  if (find_player_absolute_quiet(p->lower_name))
  {
    ...
    sp->last_host[MAX_INET_ADDR - 1] = '\0';
    strncpy(sp->email, p->email, MAX_EMAIL - 1);
    sp->email[MAX_EMAIL - 1] = '\0';
    sp->last_on = time(0);
  }
...
/* Set up a default player structure, methinks */
  ...
  strncpy(p->colorset, get_pdefaults_msg("colorset"), 9);
  p->colorset[9] = '\0';
...
void punlink(player * p)
{
  ...
  if (p->location)
  {
    ...
    if (!scan)
      pg_log("error", "Bad Location list");
    else if (!previous)
...
void link_to_program(player * p)
{
  ...
  if (!scan)
    pg_log("error", "Bad non-name hash list");
  else if (!previous)
...
/* test password — supports both legacy DES and modern SHA-512 hashes */
...
int check_password(char *password, char *entered, player * p)
{
  ...
  if (password[0] == '$')
  {
    /* modern hash — use stored hash as its own salt */
    result = crypt(entered, password);
    return (result && !strcmp(result, password));
  }
  else
  {
    /* legacy DES hash — truncate to 8 chars, use player name as salt */
    char key[9];
...
void got_password(player * p, char *str)
{
  ...
  {
    tell_player(p, "\n Your password uses outdated encryption and should be updated.\n");
    password_mode_on(p);
    do_prompt(p, " Please enter a new password: ");
    p->input_to_fn = login_upgrade_pw1;
    return;
  }
...
/* continue login after optional password upgrade or gender prompt */

void finish_login(player *p)
{
  if (p->gender < 0)
  {
    p->input_to_fn = enter_gender;
...
/* login-time password upgrade callbacks — same as got_password1/2 but
   continue the login flow instead of returning to the command loop */

void login_upgrade_pw2(player *p, char *str)
{
  password_mode_off(p);
  p->input_to_fn = 0;
  if (strcmp(p->password_cpy, str))
  {
    tell_player(p, "\n But that doesn't match!\n"
                " Password not changed, continuing login...\n");
  }
  else
  {
    strcpy(p->password, update_password(str));
...
void login_upgrade_pw1(player *p, char *str)
{
  (void)str;
  ...
  else
  {
    ...
    p->input_to_fn = login_upgrade_pw2;
  }
}
  ...
  for (charcounter = 0; charcounter < 26; charcounter++)
  {
	...
	switch (scanlist->residency)
	{
	  ...
	  case BANISHED:
	    break;
	  default:
	    strncpy(email, scanlist->email, MAX_EMAIL - 1);
	    email[MAX_EMAIL - 1] = '\0';
	    lower_case(email);
	    if (scanlist->email[0] && strstr(email, str))
	    {
	      stack = strchr(stack, 0);
  ...
  for (charcounter = 0; charcounter < 26; charcounter++)
  {
	...
	switch (scanlist->residency)
	{
	  ...
	  case BANISHED:
	    break;
	  default:
	    strncpy(lasthost, scanlist->last_host, MAX_INET_ADDR - 1);
	    lasthost[MAX_INET_ADDR - 1] = '\0';
	    lower_case(lasthost);
	    if (scanlist->last_host[0] && strstr(lasthost, str))
	    {
	      stack = strchr(stack, 0);
...
void master_search(player * p, char *str, search_type * st)
{
  ...
  *stack++ = '\n';
  ...
  for (c = 'a'; (c <= 'z' && !outnow); c++)
  {
	...
	snprintf(dummy.lower_name, MAX_NAME, "%s", scan->lower_name);
	dummy.fd = p->fd;
	if (!load_player(&dummy))
	{
...
{
  static char clc[MAX_NAME + 8];

  if (!pg_strcasestr(p->lower_name, str))
    return (char *) NULL;

  memset(clc, 0, sizeof(clc));

  if (p->residency & (CODER | ADMIN | LOWER_ADMIN))
    snprintf(clc, sizeof(clc), "^B%s^N, ", p->lower_name);
  else if (p->residency & (PSU | SU | ASU | ADC))
    snprintf(clc, sizeof(clc), "^G%s^N, ", p->lower_name);
  else
    snprintf(clc, sizeof(clc), "%s, ", p->lower_name);
...
char *site_search(player * p, char *str)
{
  ...
  if (!p->saved || !pg_strcasestr(p->saved->last_host, str))
    return (char *) NULL;
...
char *url_search(player * p, char *str)
{
  ...
  if (!pg_strcasestr(p->alt_email, str))
    return (char *) NULL;
...
char *resby_search(player * p, char *str)
{
  ...
  if (!p->ressied_by[0] || !pg_strcasestr(p->ressied_by, str))
    return (char *) NULL;
...
char *git_search(player * p, char *str)
{
  ...
  if (!pg_strcasestr(p->git_string, str) && !pg_strcasestr(p->git_by, str))
    return (char *) NULL;

  memset(clc, 0, 160);
  snprintf(clc, sizeof(clc), " %-20s (%s) %s^N\n", p->lower_name, p->git_by, p->git_string);
  return clc;
}
...
char *swarn_search(player * p, char *str)
{
  ...
  if (!pg_strcasestr(p->swarn_message, str) &&
      !pg_strcasestr(p->swarn_sender, str))
    return (char *) NULL;
