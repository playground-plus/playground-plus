...
void delete_entry(saved_player * sp, list_ent * l)
{
  ...
  while (scan)
    if (scan->next == l)
    {
      ...
      scan = scan->next;
  pg_log("error", "Tried to delete list entry that wasn't there.");
}
  ...
  while (*str)
  {
    ...
    *stack++ = 0;
    if (*oldstack)
    {
      ...
      {
	char tmp_entry[256];
	strncpy(tmp_entry, oldstack, sizeof(tmp_entry) - 1);
	tmp_entry[sizeof(tmp_entry) - 1] = 0;
	text = stack;
...
int get_flags(player * p, char *flags, flag_list * list)
{
  ...
  while (*flags)
  {
    ...
    *stack++ = 0;
    ...
    {
      char tmp_flag[256];
      strncpy(tmp_flag, oldstack, sizeof(tmp_flag) - 1);
      tmp_flag[sizeof(tmp_flag) - 1] = 0;
      text = stack;
...
char *check_legal_entry(player * p, char *str, int verbose)
{
  ...
  if (!splyr)
  {
    pg_log("error", "Spooned again chris. check_legal_entry");
    return 0;
  }
