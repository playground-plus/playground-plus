...
void vlog(player * p, char *str)
{
  ...
  if (!*str || *str == '?')
  {
    ...
    for (i = 0; i < dc; i++)
    {
      snprintf(path, sizeof(path), "logs/%s", de[i]->d_name);
...
void show_logs(player * p, char *str)
{
  struct dirent **de;
  struct stat sbuf;
  char *oldsmack = stack, path[280], tl[260], *pt;
  ...
  for (i = 0; i < dc; i++)
  {
	...
	*stack-- = 0;
    ...
    else
    {
      ...
      snprintf(tl, sizeof(tl), "%s", de[i]->d_name);
      pt = strchr(tl, 0);
      while (*pt-- != '.');
      *++pt = 0;
...
void validate_email(player * p, char *str)
{
  ...
  stack = end_string(stack);
  pg_log("validate_email", oldstack);
  stack = oldstack;
...
void make_git(player * p, char *str)
{
  ...
  strncpy(git->git_string, gitstr, MAX_DESC - 1);
  git->git_string[MAX_DESC - 1] = '\0';
  strncpy(git->git_by, p->name, MAX_NAME - 1);
  git->git_by[MAX_NAME - 1] = '\0';
  git->residency |= GIT;
...
void marry_edit(player * p, char *str)
{
  ...
  sprintf(p2->married_to, "%s", check_legal_entry(p, newmn, 0));
...
void vscript(player * p, char *str)
{
  struct dirent **de;
  struct stat sbuf;
  char *oldstack = stack, path[280], *fr;
  int dc = 0, i, vemerg = 0;
  file lf;
