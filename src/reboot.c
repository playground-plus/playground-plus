...
int build_sysinfo(void)
{
  ...
  tsi.session[MAX_SESSION - 1] = '\0';
  strncpy(tsi.sess_name, sess_name, MAX_NAME - 1);
  tsi.sess_name[MAX_NAME - 1] = '\0';
  if (current_player)
  {
    snprintf(tsi.rebooter, MAX_NAME, "%s", current_player->lower_name);
  }
...
int build_loggedin_players_list(void)
{
  ...
  IGNORE_RET(write(fd, oldstack, strlen(oldstack)));
  close(fd);
  stack = oldstack;
  return 0;
}
...
int build_loggedin_players_info(void)
{
  ...
  for (scan = flatlist_start; scan; scan = scan->flat_next)
  {
    ...
    scan->location_string[MAX_NAME + MAX_ID - 1] = '\0';
...
void trans_to_quiet(player * p, char *str)
{
  ...
  if (p->location)
  {
    ...
    if (!scan)
      pg_log("error", "Bad Location list 2");
    else if (!previous)
    {
...
int retrieve_sysinfo(void)
{
  ...
  IGNORE_RET(fread((void *) &tsi, sizeof(talker_system_info), 1, f));
  fclose(f);
...
void retrieve_players(void)
{
  ...
  if (!f)
  {
    pg_log("error", "When rebooting, in retrieve_players, fopen failed.");
    return;
  }
  memset(namein, 0, 160);
  ...
  while (ret && ret != EOF)
  {
    ...
    {
      IGNORE_RET(fread((void *) &spanky, sizeof(player), 1, pf));
      fclose(pf);
...
int possibly_reboot(void)
{
  ...
  memset(r, 0, 16);
  IGNORE_RET(fgets(r, 15, f));
  if (getpid() != atoi(r))
    return 0;

  SUWALL(" -=*> Starting reboot ...\n");
  pg_log("boot", "Rebooting ...");
  ...
  IGNORE_RET(system("rm -f " REBOOTING_DIR "/*"));
...
int reboot_save_multis(void)
{
  ...
  {
    pg_log("reboot", "Couldn't open junk/rebooting/multi for writing");
    return 1;
  }
...
void reboot_load_multis(void)
{
  ...
  {
    pg_log("reboot", "Couldn't open junk/rebooting/multi for reading");
    return;
  }
