...
void nuke_player(player * p, char *str)
{
  ...
  *reason++ = 0;
  ...
  if (p2)
  {
    ...
    TELLPLAYER(p2, "%s", nuke_msg.where);
...
void banish_player(player * p, char *str)
{
  ...
  if (strlen(str) > MAX_NAME - 2)
  {
    ...
    pg_log("banish", " ... but failed cause its tooo long");
    return;
  }
  ...
  if (p2)
  {
    if (p == p2)
    {
      ...
      pg_log("banish", " ... but failed cause you cant banish yerself");
      return;
    }
    if (!check_privs(p->residency, p2->residency))
    {
  ...
  if (!newbie)
  {
	...
	if (!isalpha(*i++))
	{
	  ...
	  pg_log("banish", " ... but failed cause its got non alphas");
	  return;
	}
      }
...
/* chop the argument into "name\000email\000password\000" with ptrs as
  ...
  *stack++ = 0;
  ...
  destroy_player(np);
  {
    ...
    strncpy(tmp_name, oldstack, MAX_NAME - 1);
    tmp_name[MAX_NAME - 1] = 0;
    cpy = stack;
    sprintf(cpy, "%s creates %s.", p->name, tmp_name);
  }
...
void do_rename(player * p, char *str, int verbose)
{
  ...
  *firspace = 0;
  ...
  if (!scan)
    pg_log("error", "Bad hash list (rename)");
  else if (!previous)
...
void marry(player * p, char *str)
{
  ...
  *temp++ = 0;
  ...
  strncpy(p2->married_to, p3->name, MAX_NAME - 1);
  p2->married_to[MAX_NAME - 1] = '\0';
  strncpy(p3->married_to, p2->name, MAX_NAME - 1);
  p3->married_to[MAX_NAME - 1] = '\0';
  save_player(p2);
  save_player(p3);
...
void net_propose(player * p, char *str)
{
  ...
  strncpy(p->married_to, q->name, MAX_NAME - 1);
  p->married_to[MAX_NAME - 1] = '\0';
