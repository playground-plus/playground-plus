...
void remove_any_note(note * n)
{
  ...
  else
  {
    ...
    if (!sp)
    {
      pg_log("mail", "Bad owner name in mail(1)");
      pg_log("mail", oldstack);
...
void sync_notes(int background)
{
  ...
  if (sys_flags & VERBOSE || sys_flags & PANIC)
    pg_log("sync", "Dumping notes to disk");
  for (n = 0; n < NOTE_HASH_SIZE; n++)
    sync_note_hash(n);
  if (sys_flags & VERBOSE || sys_flags & PANIC)
    pg_log("sync", "Note dump completed");
...
void init_notes()
{
  ...
  if (sys_flags & VERBOSE || sys_flags & PANIC)
    pg_log("boot", "Loading notes from disk");
...
void end_mail(player * p)
{
  ...
  if (receipt_count > 0)
  {
    ...
    {
      char tmp_name_list[1024];
      strncpy(tmp_name_list, name_list, sizeof(tmp_name_list) - 1);
      tmp_name_list[sizeof(tmp_name_list) - 1] = 0;
      if (mail->flags & ANONYMOUS)
	sprintf(stack, "\nDate: %s"
		"\nFrom: <anonymous>"
		"\nTo: %s"
		"\nSubject: %s^N\n\n", convert_time(mail->date), tmp_name_list, mail->header);
      else
	sprintf(stack, "\nDate: %s"
		"\nFrom: %s"
		"\nTo: %s"
		"\nSubject: %s^N\n\n", convert_time(mail->date), mail->name, tmp_name_list, mail->header);
    }
    stack = strchr(stack, 0);
