...
int main(int argc, char *argv[])
{
  ...
  while (!die)
  {
    ...
    {
      pg_log("error", "Crashing lots!! - Giving up.");
      exit(-1);
    }
    else if (time_out < t)
    {
    ...
    pg_log("angel", "Playground+ guardian angel bootloader v1.7 (19.07.99)");
    dieing = 0;
    fh = fork();
    switch (fh)
    {
	...
	{
	  kill(fh, SIGKILL);
	  pg_log("angel", "Killed server before connect");
	  waitpid(fh, &status, 0);
	}
	else
	{
	  ...
	  while (waitpid(fh, &status, WNOHANG) <= 0)
	  {
		...
		{
		  kill(fh, SIGKILL);
		  pg_log("angel", "Server KILLED");
		}
		else
		{
		  kill(fh, SIGTERM);
		  pg_log("angel", "Server TERMINATED");
		  dieing = 1;
		}
	      }
	    }
		...
		error("bad FIONREAD");
	      ...
	      {
		kill(fh, SIGKILL);
		pg_log("angel", "Server disconnected");
		dieing = 1;
	      }
	      else
	      {
		for (; length; length--)
		{
		  IGNORE_RET(read(alive_fd, &dummy, 1));
		}
	      }
	    }
	  }
	...
	{
	  case 0:
	    pg_log("angel", "Server exited safely");
	    break;
	  case 127:
	  ...
	  default:
	    ...
	    stack = stack_start;
	    if (status & 128)
	      pg_log("angel", "Core dump produced - oops!?");
	    break;
	}
	break;
    }
...
char *get_config_msg(char *type)
{
  ...
  {
    pg_log("error", "Angel : Softmsg file for config_msg aint loaded!");
    return "error";
  }
  got = lineval(config_msg.where, type);
  if (!got || !*got)
  {
    pg_log("error", "Angel : Softmsg in config_msg isnt there!");
    return "error";
  }
