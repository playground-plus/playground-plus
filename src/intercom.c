...
static void icom_log (const char *filename, const char *string)
{
  int fd, length;
...
static void close_intercom_main_socket (void)
{
  if (inet_fd > 0)
  {
      ...
      close (inet_fd);
      icom_log ("intercom", "Closing main file descriptor");
    inet_fd = -1;
      close_all_sockets ();
  }
  return;
...
{
  icom_log ("intercom", "Hangup received. Dying");
  closedown = 1;
  return;
}
...
{
  /* Reap zombie child processes. Do NOT call fatal_error here —
     SIGCHLD fires on normal child exit, not just crashes. */
  (void)dummy;
...
static void parse_hello (talker_list * remote_talker, char *str)
{
  ...
  char tmp_name[MAX_TALKER_NAME + 1];
    ...
    {
      {
        ...
        if (getaddrinfo(dynamic, NULL, &hints, &res) != 0 || !res)
          dynamic = 0;
        else
          {
            memcpy(&inet_address, &((struct sockaddr_in *)res->ai_addr)->sin_addr, sizeof(struct in_addr));
            freeaddrinfo(res);
  ...
  if (!known)
  {
    ...
    {
	  strcpy (tmp_name, remote_talker->name);
      tmp_name[MAX_TALKER_NAME - 2] = 0;
      if (name_match > 9)
        tmp_name[MAX_TALKER_NAME - 3] = 0;
      if (name_match > 99)
        tmp_name[MAX_TALKER_NAME - 4] = 0;
      if (name_match > 999)
        tmp_name[MAX_TALKER_NAME - 5] = 0;

	  snprintf (dummy_name, sizeof (dummy_name), "%s%d", tmp_name, name_match);
...
static void parse_validation_request (talker_list * remote_talker, char *str)
{
  ...
  if (!str || !*str)
  {
      icom_log ("intercom", "Empty validation string");
      I_SHUTDOWN (remote_talker->fd, 2);
      do_close (remote_talker);
  ...
  {
      icom_log ("intercom", "Incorrect validation string");
      I_SHUTDOWN (remote_talker->fd, 2);
      do_close (remote_talker);
  ...
  if (!known)
  {
    /*This was a validation request from an unknown server. Kill it */
      icom_log ("intercom", "Unknown talker sent validation. Killed.");
...
static void check_validation (talker_list * remote_talker, char *str)
{
  ...
  if (!str || !*str)
  {
      icom_log ("intercom", "Empty validation reply string");
      I_SHUTDOWN (remote_talker->fd, 2);
      do_close (remote_talker);
  ...
  if (!validation_str || !*validation_str || !*port_str)
  {
      icom_log ("intercom", "Incorrect validation reply string");
      I_SHUTDOWN (remote_talker->fd, 2);
      do_close (remote_talker);
  ...
  {
    /*This was a validation request from an unknown server. Kill it */
      icom_log ("intercom", "Unknown talker sent reply validation. Killed.");
...
static void parse_in_user_tell_command (talker_list * remote_talker, char *str)
{
  ...
  if (!str || !*str)
  {
      icom_log ("intercom", "Bad user message sent");
    return;
  }
  job_str = str;
  str = strchr (job_str, ':');
...
static void parse_in_user_say_command (talker_list * remote_talker, char *str)
{
  ...
  {
      icom_log ("intercom", "Invalid string in parse_in_user_say_command");
    return;
  }
...
static void parse_in_user_emote_command (talker_list * remote_talker, char *str)
{
  ...
  if (!name || !*name || !msg || !*msg)
  {
      icom_log ("intercom", "Invalid string in parse_in_user_emote_command");
    return;
  }
  /*Build a reply string */
  oldstack = stack;
...
static void parse_reply_from_remote (talker_list * remote_talker, char *str)
{
  ...
  if (!str || !*str || !*(str + 1))
  {
      icom_log ("intercom", "Bad reply received for remote command result.");
    return;
  }
  name = strchr (str + 1, ':');
  if (name)
  ...
  if (!name || !*name)
  {
      icom_log ("error", "Invalid format in parse_reply_from_remote");
    return;
  }
  if (!ptr || !*ptr)
    ptr = blank_field;
...
static void get_remote_list (talker_list * remote_talker, char *str)
{
  ...
  char tmp_name[MAX_TALKER_NAME];
  char tmp_abbr[MAX_TALKER_ABBR];
  talker_list *scan;
  ...
  strncpy (tmp_name, name, sizeof (tmp_name) - 1);
  tmp_name[sizeof (tmp_name) - 1] = 0;
  strncpy (tmp_abbr, abbr, sizeof (tmp_abbr) - 1);
  tmp_abbr[sizeof (tmp_abbr) - 1] = 0;
  oldstack = stack;
  sprintf(oldstack,"%s:%s:%d.%d.%d.%d:%s:F:%ld:%s",tmp_name,tmp_abbr,num[0],
  ...
  if (add_new_talker (oldstack, 0))
  {
    sprintf (oldstack, "Talker '%s' just informed us of talker '%s'. Added to the"
  	    " database as a banished talker.", remote_talker->name, tmp_name);
    stack = end_string (oldstack);
    tell_talker_su (oldstack);
  }
  stack = oldstack;
...
static void return_room_look (talker_list * remote_talker, char *str)
{
  ...
  {
      icom_log ("intercom", "Bad string in return_room_look");
    return;
  }
...
static void informed_move (talker_list * remote_talker, char *str)
{
  ...
  {
      icom_log ("intercom", "Bad string from informed_move");
    return;
  }
  I_SHUTDOWN (remote_talker->fd, 2);
  do_close (remote_talker);
  ...
  if (all_num)
  {
    ...
    if (inet_address.s_addr != (in_addr_t)-1)
    {
      ...
      if (getnameinfo((struct sockaddr *)&sa, sizeof(sa),
        ...
        strcpy(remote_talker->addr, namebuf);
      else
  ...
  else
  {
    /*Its an alpha address, we need to convert */
    {
      ...
      hints.ai_family = AF_INET;
      hints.ai_socktype = SOCK_STREAM;
      if (getaddrinfo(site, NULL, &hints, &res) != 0 || !res)
        return;
      memcpy(&inet_address, &((struct sockaddr_in *)res->ai_addr)->sin_addr, sizeof(struct in_addr));
      freeaddrinfo(res);
      strcpy(numeric_string, inet_ntoa(inet_address));
...
static void parse_remote_talker_input (talker_list * remote_talker, int validated)
{
  ...
  if (ioctl (remote_talker->fd, FIONREAD, &chars_left) == -1)
  {
      ...
      icom_log ("intercom", "PANIC on FIONREAD on remote_talker->fd.");
  ...
  stack = oldstack + this_packet->length;

  while (this_packet->next)
  {
    this_packet = this_packet->next;
    this_packet->data[this_packet->length] = '\0';
    memcpy(stack, this_packet->data, this_packet->length + 1);
    stack += this_packet->length;
  }
  stack++;
...
static int get_new_talker_connection (void)
{
  ...
  if (new_fd < 0)
  {
      icom_log ("intercom", "Failed to accept on inet_fd");
    return -1;
  }
  if (ioctl (new_fd, FIONBIO, &dummy) < 0)
  {
      icom_log ("intercom", "Failed to set non-blocking on incoming"
	" remote talker.");
      I_SHUTDOWN (new_fd, 2);
      close (new_fd);
    return 0;
  ...
  {
    char namebuf[NI_MAXHOST];
    struct sockaddr_in sa_copy;
    memset(&sa_copy, 0, sizeof(sa_copy));
    sa_copy.sin_family = AF_INET;
    memcpy(&sa_copy.sin_addr, &sa.sin_addr, sizeof(sa.sin_addr));
    if (getnameinfo((struct sockaddr *)&sa_copy, sizeof(sa_copy),
                    namebuf, sizeof(namebuf), NULL, 0, 0) == 0)
      strcpy(new_var->addr, namebuf);
    else
...
static int create_main_intercom_socket (const char *path)
{
  ...
  if (unix_fd < 0)
  {
      icom_log ("intercom", "Failed to create unix socket.");
    return -1;
  }
  ...
  {
      icom_log ("intercom", "Failed to bind to unix socket.");
    return -1;
  }
  if (listen (unix_fd, 1) < 0)
  {
      icom_log ("intercom", "Failed to listen at unix socket");
  ...
  icom_log ("intercom", "Unix socket bound and listening.");
...
static int get_unix_fd_input (void)
{
  ...
  if (talker_fd < 0)
  {
      icom_log ("intercom", "Failed to accept on unix_fd");
    return -1;
  }
  icom_log ("intercom", "Connected intercom to talker");
...
void send_to_talker (const char *fmt,...)
{
  ...
  if (!fmt || !*fmt)
  {
      icom_log ("intercom", "Empty string in send_to_talker");
    return;
  }
  va_start (varlist, fmt);
  vsnprintf (stack, INTERCOM_STACK, fmt, varlist);
  va_end (varlist);

  if (!*oldstack)
  {
      icom_log ("intercom", "Tried to send null string to server.");
    return;
  }
  stack = strchr (oldstack, '\0');
...
static void request_port_from_talker (void)
{
  ...
  icom_log ("intercom", "Requesting port number from talker");
...
int tell_remote_talker (talker_list * remote_talker, const char *fmt,...)
{
  ...
  if (!fmt || !*fmt)
  {
      icom_log ("intercom", "Tried to write NULL to remote talker");
    return -1;
  }
  ...
  va_start (varlist, fmt);
  vsnprintf (oldstack, INTERCOM_STACK, fmt, varlist);
  va_end (varlist);
  ...
  *stack++ = '\0';
  ...
  ptr = oldstack;
  while (*ptr && strlen (ptr) >= MAX_PACKET)
  {
  ...
  if (*ptr)
  {
    size_t remaining = strlen (ptr);
      remote_talker->net_stats.chars_out += remaining;
    remote_talker->net_stats.packets_out++;
      total_net.chars_out += remaining;
    total_net.packets_out++;
      if (write (remote_talker->fd, ptr, remaining) == -1)
      fail = 1;
  }
...
static void send_hello (talker_list * remote_talker)
{
  ...
  *stack++ = 0;
  ...
  tell_remote_talker(remote_talker, "%s", oldstack);
  stack = oldstack;
}
...
int connect_new_talker (talker_list * new_link)
{
  ...
  if (new_link->fd < 0)
  {
      icom_log ("intercom", "Failed to create socket for remote link");
    return 0;
  }
  dummy = 1;
...
static void create_inet_socket (char *str)
{
  ...
  if (!*str)
  {
      icom_log ("intercom", "Talker sent zero length portnumber. Re-requesting.");
  ...
  if (inet_fd < 0)
  {
      icom_log ("intercom", "Couldnt grab inet_fd socket");
    inet_fd = -1;
    return;
  }
  dummy = 1;
  if (setsockopt (inet_fd, SOL_SOCKET, SO_REUSEADDR, (char *) &dummy, sizeof (dummy))
      < 0)
    icom_log ("intercom", "Couldnt set reuse address on inet_fd, ignoring.");
  lingerval.l_onoff = 1;
  lingerval.l_linger = 0;
  ...
  {
      icom_log ("intercom", "Couldnt set non-blocking on inet_fd.");
      close (inet_fd);
    inet_fd = -1;
    return;
  }
  ...
  {
    local_errno = errno;
      icom_log ("intercom", "Couldnt bind inet_fd to port");
    switch (local_errno)
    {
      case EBADF:
	  icom_log ("intercom", "Bad file descriptor");
	break;
      case EINVAL:
	  icom_log ("intercom", "Already bound.");
	break;
      case EACCES:
	  icom_log ("intercom", "Privillaged port");
	break;
      case EADDRINUSE:
	  icom_log ("intercom", "Address in use.");
	break;
      default:
	  icom_log ("intercom", "errno failed.");
	break;
    }
      I_SHUTDOWN (inet_fd, 2);
      close (inet_fd);
  ...
  {
      icom_log ("intercom", "Failed on listen. Closing inet_fd.");
      I_SHUTDOWN (inet_fd, 2);
      close (inet_fd);
    inet_fd = -1;
      close_all_sockets ();
...
void tell_personal_inc_ref (job_list * this_job, const char *fmt,...)
{
  ...
  if (current_name[0] == '\0')
  {
      icom_log ("intercom", "No name to send personal message to.");
    return;
  }
  oldstack = stack;
...
int add_new_talker (char *str, int verbose)
{
  ...
  if (all_num)
  {
    ...
    if (inet_address.s_addr != (in_addr_t)-1)
    {
      ...
      if (getnameinfo((struct sockaddr *)&sa, sizeof(sa),
                      namebuf, sizeof(namebuf), NULL, 0, 0) == 0)
        strcpy(new_talker->addr, namebuf);
      else
  ...
  else
  {
    /*Its an alpha address, we need to convert */
    {
      ...
      hints.ai_family = AF_INET;
      hints.ai_socktype = SOCK_STREAM;
      if (getaddrinfo(new_talker->addr, NULL, &hints, &res) != 0 || !res)
      {
        new_talker->fd = -1;
        if (verbose > 0)
...
static void add_new_server_to_file (char *str)
{
  ...
  *abbr = ':';
  ...
  if (fp)
  {
    ...
    icom_log ("intercom", "Failed to open database list for addition");
...
static void read_talker_database (void)
{
  ...
  if (fp)
  {
    ...
    {
      buffer[0] = '\0';
	  IGNORE_RET(fgets(buffer, 254, fp));
	  ptr = strchr (buffer, '\n');
      if (ptr)
	*ptr = '\0';
      if (buffer[0])
    ...
    icom_log ("intercom", "Tried to open list of talkers but failed");
...
char *set_name (char *str)
{
  ...
  {
    *ptr++ = '\0';
      strncpy (current_name, str, sizeof (current_name) - 1);
...
static void show_all_links (int hidden, int up_only)
{
  ...
  sprintf (stack, "\nMain server connection status : %s----------------------"
	  "------------------\n",
...
{
  char *target_talker, *target_name, *msg = 0;
  ...
  if (!str || !*str || !*(str + 1))
  {
      icom_log ("intercom", "User command with no body");
    return;
  }
  command_style = str;
  str++;
				       ...
				       char *str, char c_type)
{
  if (!str || !*str)
  {
      icom_log ("intercom", "Returned empty who list from server");
    return;
  }
...
static void parse_server_reply (char *str)
{
  ...
  if (!str || !*str)
  {
      icom_log ("intercom", "Talker sent empty reply to request");
    return;
  }
  ptr = 0;
  ...
  if (!name || !*name)
  {
      icom_log ("intercom", " Invalid line to parse in parse_server_reply");
    return;
  }
  if (!ptr || !*ptr)
    ptr = blank_field;
  ...
  {
      icom_log ("intercom", "No return talker address in reply request.");
    return;
  }
...
void sync_talkers ()
{
  ...
  if (!fp)
  {
      icom_log ("intercom", "ERROR: Couldnt open intercom database file.");
    return;
  }
  scan = talker_list_anchor;
...
static void delete_link (char *str)
{
  ...
  if (!str || !*str)
  {
      icom_log ("intercom", "No talker name specified to delete");
    return;
  }
...
static void change_name (char *str)
{
  ...
  {
      icom_log ("intercom", "Bad message passed to change_name");
    return;
  }
...
static void change_abbr (char *str)
{
  ...
  if (!new_var || !*new_var || !str || !*str)
  {
      icom_log ("intercom", "Bad message passed to change_abbr");
    return;
  }
...
static void change_addr (char *str)
{
  ...
  if (!new_var || !*new_var || !str || !*str)
  {
      icom_log ("intercom", "Bad message passed to change_address");
    return;
  }
  target = match_talker_name (str);
  if (!target)
  ...
  else
  {
    /*Its an alpha address, we need to convert */
    {
      ...
      hints.ai_family = AF_INET;
      hints.ai_socktype = SOCK_STREAM;
      if (getaddrinfo(new_var, NULL, &hints, &res) != 0 || !res)
      {
...
static void change_port (char *str)
{
  ...
  if (!new_var || !*new_var || !str || !*str)
  {
      icom_log ("intercom", "Bad message passed to change_port");
    return;
  }
  port = ATOI (new_var);
  if (port < 1)
...
static void banish_site (char *str)
{
  ...
  if (!str || !*str)
  {
      icom_log ("intercom", "Sent bad string to banish_site()");
    return;
  }
  target = match_talker_name (str);
  if (!target)
...
static void unhide_entry (char *str)
{
  ...
  if (!str || !*str)
  {
      icom_log ("intercom", "Sent bad string to unhide_entry()");
    return;
  }
  target = match_talker_name (str);
  if (!target)
...
static void hide_entry (char *str)
{
  ...
  if (!str || !*str)
  {
      icom_log ("intercom", "Sent bad string to hide_entry()");
    return;
  }
  target = match_talker_name (str);
  if (!target)
...
static void expire_jobs (void)
{
  ...
  while (scan->next != job_anchor && scan->next->timeout < system_time)
  {
    ...
    if (scan->timeout < system_time)
    {
      if (*(scan->sender))
      {
	      sprintf (current_name, "%s", scan->sender);
...
static void reply_room_look_global (char *str)
{
  ...
  {
      icom_log ("intercom", "Invalid message sent to reply_room_look_global");
    return;
  }
  this_job = return_job (job_str);
  if (!this_job)
...
static void parse_talker_input (void)
{
  ...
  if (ioctl (talker_fd, FIONREAD, &chars_left) == -1)
  {
    ...
    closedown = 1;
      ...
      icom_log ("intercom", "PANIC on FIONREAD on talker_fd.");
  ...
  if (!chars_left)
  {
    ...
    talker_fd = -1;
    closedown = 1;
      icom_log ("intercom", "Link died on talker_fd");
    return;
  }
  ...
  while (chars_left && c != (char) END_MESSAGE)
  {
    ...
    {
	  icom_log ("intercom", "Read error on talker_fd socket");
	  I_SHUTDOWN (talker_fd, 2);
	  close (talker_fd);
      talker_fd = -1;
      closedown = 1;
...
void get_config (void)
{
  ...
  icom_log ("intercom", "Intercom booting");

  if (strchr (get_config_msg ("talker_name"), ':'))
  {
      icom_log ("intercom", "In soft/config.msg, talker_name may NOT have a : in it. Not booted");
      exit (-1);
  }
  if (strchr (get_config_msg ("intercom_abbr"), ':'))
  {
      icom_log ("intercom", "In soft/config.msg, intercom_abbr may NOT have a : in it. Not booted");
      exit (-1);
  }
  ...
  if (strchr (get_config_msg ("talker_name"), '^'))
  {
      icom_log ("intercom", "Colour in your talker_name causes problems on other talkers. Intercom not booted");
      exit (-1);
  }
  if (strchr (get_config_msg ("intercom_abbr"), ':'))
  {
      icom_log ("intercom", "Colour in your intercom_abbr causes problems on other talkers. Intercom not booted");
      exit (-1);
  }
  ...
  {
      icom_log ("intercom", "Your talker name needs to be changed from 'Playground Plus'. Intercom not booted.");
      exit (-1);
  }
  if (!strcasecmp (get_config_msg ("intercom_abbr"), "pgp"))
  {
      icom_log ("intercom", "Your intercom alias needs to be changed from 'pgp'. Intercom not booted");
      exit (-1);
  }
...
static void getDynamicHost (void)
{
  ...
  if (fp)
    {
      hostname[0] = 0;
      IGNORE_RET(fgets(hostname, 254, fp));
      if (hostname[0])
	{
...
/****END SIGNALS*********/
  ...
  if (chdir (ROOT))
  {
      icom_log ("intercom", " Cannot change to root directory.");
      exit (1);
  }
...
/*get the dynamic hostname */
  ...
  if (failed == 5)
  {
      icom_log ("intercom", "Repeated fails to create unix socket, aborting.");
      exit (1);
  }
  setup_jobs_list ();
  setup_free_packets ();
  ...
  while (!closedown)
  {
    ...
    {
      stack = stack_start;
	  icom_log ("intercom", "Bad stack in main loop. Recovered.");
    }
    ...
    timeout.tv_sec = 0;
    timeout.tv_usec = (long)((TIMER_CLICK) > 0 ? (1000000 / (TIMER_CLICK)) : 200000);
  ...
  icom_log ("intercom", "Shutdown complete.");
...
file load_file (char *filename)
{
  ...
  if (f.length > 0 && read (d, f.where, f.length) < 0)
  {
...
char *get_config_msg (char *type)
{
  ...
  {
      icom_log ("error", "Intercom : Softmsg file for config_msg aint loaded!");
      exit (-1);
  }
  got = lineval (config_msg.where, type);
  if (!got || !*got)
  {
      icom_log ("error", "Intercom : Softmsg in config_msg isnt there!");
      exit (-1);
  }
...
static void make_unique_but (talker_list * safe, talker_list * remote_talker)
{
  ...
  char tmp_name[MAX_TALKER_NAME];
  char tmp_abbr[MAX_TALKER_ABBR];
  char build_abbr[MAX_TALKER_ABBR + 16];
  int loopa;
  talker_list *match;
  ...
  while (match && match != safe)
	...
	tmp_name[MAX_TALKER_NAME - 5] = 0;
      ...
      snprintf (dummy_name, sizeof (dummy_name), "%s%d", tmp_name, loopa);
      match = match_talker_name (dummy_name);
    }
  ...
  while (match && match != safe)
    {
      loopa++;
      strcpy (tmp_abbr, remote_talker->abbr);
      tmp_abbr[MAX_TALKER_ABBR - 2] = 0;
      if (loopa > 9)
	tmp_abbr[MAX_TALKER_ABBR - 3] = 0;
      if (loopa > 99)
	tmp_abbr[MAX_TALKER_ABBR - 4] = 0;
      if (loopa > 999)
	tmp_abbr[MAX_TALKER_ABBR - 5] = 0;

      snprintf (build_abbr, sizeof (build_abbr), "%s%d", tmp_abbr, loopa);
      strncpy (dummy_abbr, build_abbr, sizeof (dummy_abbr) - 1);
      dummy_abbr[sizeof (dummy_abbr) - 1] = 0;
      match = match_talker_abbr_absolute (dummy_abbr);
    }
