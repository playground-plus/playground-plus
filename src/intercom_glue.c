...
static void setup_namebans (void)
{
  ...
  {
    *stack = 0;
      IGNORE_RET(fgets (stack, MAX_NAME + 3, fp));
...
void start_intercom ()
{
  ...
  pg_log("intercom", "Forking to boot Intercom Server.");
  intercom_pid = fork ();
  ...
  switch (intercom_pid)
  {
	...
	if (scan->fd > 0)
	{
      ...
      pg_log("intercom", "Error, failed to exec intercom");
      exit (1);
      break;
    case -1:
      pg_log("intercom", "Error, failed to fork main server");
      break;
  }
...
void kill_intercom ()
{
  ...
  if (!(sys_flags & SHUTDOWN) && (!current_player))
    pg_log("error", "Shutting down intercom server.\n");
...
int establish_intercom_server ()
{
  ...
  if (this_fd < 0)
  {
      pg_log("error", "Failed to create talker to intercoms unix socket");
    return intercom_fd - 1;
  }
  sa.sun_family = AF_UNIX;
  snprintf (sa.sun_path, sizeof (sa.sun_path), "%s", INTERCOM_SOCKET);

  if (connect (this_fd, (struct sockaddr *) &sa, sizeof (sa)) < 0)
  {
    if (intercom_fd == -2)
	pg_log("error", "Failed to connect to intercom unix socket");
      close (this_fd);
    return intercom_fd - 1;
  }
  return this_fd;
...
void send_to_intercom (player * p, const char *fmt,...)
{
  ...
  vsnprintf (stack, STACK_SIZE, fmt, varlist);
  va_end (varlist);
  ...
  if (intercom_fd > 0)
    IGNORE_RET(write (intercom_fd, oldstack, strlen (oldstack)));
...
static void intercom_tell_player (char *str, int format)
{
  ...
  if (!str || !*str)
  {
      pg_log("error", "Empty message in intercom_tell_player");
    return;
  }
  msg = strchr (str, ':');
  if (msg)
...
static void intercom_tell_player_and_return (char *str)
{
  ...
  if (!ptr)
  {
      pg_log("error", "intercom tried to send to player without a valid name.");
    return;
  }
  end_pos = ptr + 1;
...
static void parse_user_command_examine_in (char *str)
{
  ...
  {
      pg_log("error", "Bad format in examine from intercom.\n");
    return;
  }
  if (!strcasecmp (name, "me"))
  {
...
static void parse_user_command_finger_in (char *str)
{
  ...
  if (!name || !*name || !job_id || !*job_id)
  {
      pg_log("error", "Bad format in finger from intercom.\n");
    return;
  }
  if (!strcasecmp (name, "me"))
  {
...
static void parse_user_command_lsu_in (char *str)
{
  ...
  if (!job_id || !*job_id)
  {
      pg_log("error", "Bad format in lsu from intercom.\n");
    return;
  }
...
static void parse_user_command_locate_in (char *str)
{
  ...
  if (!name || !*name || !job_id || !*job_id)
  {
      pg_log("error", "Bad format in parse_user_command_locate_in");
    return;
  }
...
static void parse_user_command_idle_in (char *str)
{
  ...
  if (!name || !*name || !job_id || !*job_id)
  {
      pg_log("error", "Bad format in idle from intercom.\n");
    return;
  }
  if (!strcasecmp (name, "me"))
  {
...
void parse_incoming_intercom ()
{
  ...
  if (ioctl (intercom_fd, FIONREAD, &chars_left) == -1)
  {
    ...
    intercom_fd = -1;
      ...
      pg_log("error", "PANIC on FIONREAD on intercom_fd.");
  ...
  if (!chars_left)
  {
      ...
      close (intercom_fd);
    intercom_fd = -1;
      pg_log("error", "Link died on intercom_fd");
    return;
  }
  oldstack = stack;
  ...
  while (chars_left && c != (char) END_MESSAGE)
  {
    ...
    {
	  pg_log("error", "Read error on intercom unix socket");
	  shutdown (intercom_fd, 2);
	  close (intercom_fd);
      intercom_fd = -1;
      return;
...
void do_intercom_who (player * p, char *str)
{
  ...
  if (!*str || *str != '@' || !*(str + 1))
  {
      pg_log("error", "Intercom who called with invalid arg");
    return;
  }
...
void intercom_banish_name (player * p, char *str)
{
  ...
  if (!fp)
  {
      pg_log("error", "Error opening intercom banish file for writing.\n");
    return;
  }
  fprintf (fp, "%s\n", str);
  fclose (fp);
...
static void sync_banish_names (void)
{
  ...
  if (!fp)
  {
      pg_log("error", " Error opening interban file for writing.\n");
    return;
  }
  scan = nameban_anchor;
...
void do_intercom_say (player * p, char *str)
{
  ...
  if (ptr < str)
  {
      pg_log("error", "Zero length string passed to do_intercom_say\n");
    return;
  }
  switch (*ptr)
  {
...
void do_intercom_emote (player * p, char *str)
{
  ...
  if (!str || !*str)
  {
      pg_log("error", "Zero length string passed to do_intercom_emote\n");
    return;
  }
  if (*str == '\'')
...
void do_intercom_think (player * p, char *str)
{
  ...
  if (!str || !*str)
  {
      pg_log("error", "Zero length string passed to do_intercom_think\n");
    return;
  }
  ...
  if (!fp)
    {
      ...
      pg_log("error", "Failed to write to files/intercom.dynamic");
      return;
    }
