...
void	ewe_reindex_lines(ed_info *info)
{
  ...
  if(!info) {
    pg_log("new_editor", "Severe fuckwittedness in ewe_reindex_lines");
    return;
  }
...
editor_line	*insert_edit_line(char *line, ed_info *info, int before)
{
  ...
  if(!new) {
    pg_log("new_edit", "Failed to malloc for line");
    return 0;
  }
...
void            editor_main(player * p, char *str)
{
   if (!p->edit_info)
   {
      pg_log("error", "Editor called with no edit_info");
      return;
   }
   if (*str == '/')
   {
...
void            edit_goto_line(player * p, char *str)
{
   ...
   pg_log("new_editor", "meep!  go function failed to locate a line");
...
void	ewe_move_marked(ed_info *e)
{
  ...
  if(!scan) {
    pg_log("new_editor", "ewe_move_marked broken 1");
    return;
  }
  mstart = scan;
  mend = scan;
  ...
  if(e->current_line->flags & edlineMARKED1) {
    if(mend->next)
      ...
      pg_log("new_editor", "ewe_move_marked broken 2");
      return;
    }
  }
...
void	edit_replace_lines(player *p, char *str)
{
  ...
  if(!scan) {
    pg_log("new_editor", "fucked in replace 1");
