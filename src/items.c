...
void delete_entry_item(saved_player * sp, item * l)
{
  ...
  while (scan)
    if (scan->next == l)
    {
      ...
      scan = scan->next;
  pg_log("error", "Tried to delete item that wasn't there.");
}
...
void tmp_comp_item(saved_player * sp)
{
  ...
  while (l)
  {
    ...
    if (l->id < 0)
    {
      pg_log("error", "Bad item entry on compress .. auto deleted.");
...
void init_sitems()
{
  ...
  if (sys_flags & VERBOSE || sys_flags & PANIC)
    pg_log("boot", "Loading items from disk");
