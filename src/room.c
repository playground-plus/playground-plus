...
room *create_room(player * p)
{
  ...
  snprintf(id, sizeof(id), "room.%d", unique);
  do
  {
...
{
  char *exits, *scan, *oldstack, *text, newid[80];
...
void init_rooms()
{
  ...
  if (sys_flags & VERBOSE)
    pg_log("boot", "Common rooms loaded.");
}
