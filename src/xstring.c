 ...
 */
char *pg_strcasestr(char *haystack, char *needle)
{
  char s[5], *nptr, *hptr;
...
char *strcaseline(char *haystack, char *needles)
{
  ...
  for (memset(ln, 0, 160); *lf; memset(ln, 0, 160), i = 0)
  {
    while (*lf && *lf != '\n' && i < 158)
      ln[i++] = *lf++;
    if ((re = pg_strcasestr(haystack, ln)))
      return re;
    if (*lf == '\n')
      lf++;
  }
...
char *replace(char *str, char *find, char *rep)
{
  ...
  {
    /* seed */
    snprintf(returnstr, sizeof(returnstr), "%s", single_replace(str, find, rep));

    /* recursion */
    while (strstr(returnstr, find))
      snprintf(returnstr, sizeof(returnstr), "%s", single_replace(returnstr, find, rep));
...
char *linestr(char *haystacks, char *needle)
{
  ...
  ptr = haystacks;
  while ((ptr = pg_strcasestr(ptr, needle)))
