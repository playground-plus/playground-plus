...
int init_ident_server(void)
{
	...
	if (-1 == pipe(ident_toclient_fds))
	{
		...
		{
			case EMFILE:
				pg_log("boot", "init_ident_server: Too many fd's in use by"
		 		" process");
				exit(1);
			case ENFILE:
				pg_log("boot", "init_ident_server: Too many fd's in use in"
		 		" system");
				exit(1);
			case EFAULT:
				pg_log("boot", "init_ident_server: ident_toclient_fds invalid!");
				exit(1);
		}
	}
	if (-1 == pipe(ident_toserver_fds))
	{
		switch (errno)
		{
			...
			case ENFILE:
				...
				exit(1);
			case EFAULT:
				pg_log("boot", "init_ident_server: ident_toserver_fds invalid!");
				exit(1);
		}
	}
	...
	{
		case -1: /* Error */
			pg_log("boot", "init_ident_server couldn't fork!");
			exit(1);
		case 0:   /* Child */
			close(IDENT_CLIENT_READ);
			close(IDENT_CLIENT_WRITE);
			close(0);
			IGNORE_RET(dup(IDENT_SERVER_READ));
			close(IDENT_SERVER_READ);
			close(1);
			IGNORE_RET(dup(IDENT_SERVER_WRITE));
			close(IDENT_SERVER_WRITE);
			execlp("bin/ident_server", namebuffer, (char *)NULL);
			pg_log("boot", "init_ident_server failed to exec ident_server");
			exit(1);
		default: /* Parent */
	 ...
	 {
		pg_log("error", "Ack! Can't set non-blocking to ident read.");
	 }
	 if (ioctl(IDENT_CLIENT_WRITE, FIONBIO, &dummy) < 0)
	 {
		pg_log("error", "Ack! Can't set non-blocking to ident write.");
	 }
#endif /* FIONBIO */
	}
	...
	while (-1 == (ret = select(FD_SETSIZE, &fds, 0, 0, &timeout)))
	{
		...
		pg_log("boot", "init_ident_server: Timed out waiting for server"
				  " connect");
		kill_ident_server();
		return 0;
	}
	...
	if (strcmp(ident_buf_input, IDENT_SERVER_CONNECT_MSG))
	{
		...
		pg_log("boot", "init_ident_server: Bad connect from server, killing");
		kill_ident_server();
		return 0;
	}
	pg_log("boot", "Ident Server Up and Running");
...
fprintf(stderr, "send_ident_request: sockname = '%s'\n",
	...
	*s++ = IDENT_CLIENT_SEND_REQUEST;
	...
	{
		pg_log("ident", "Client failed to write request, killing and restarting"
					" Server");
		kill_ident_server();
	...
	for (i = 0 ; i < msg_size ;)
	{
		switch (reply_buf[i++])
		{
		 ...
		 if (c)
		 {
			 ...
			 *t = '\0';
#if !defined(LOTHLORIEN)
					strncpy(c->remote_user, reply, MAX_REMOTE_USER - 1);
				c->remote_user[MAX_REMOTE_USER - 1] = '\0';
#endif /* !LOTHLORIEN */
#if defined(DEBUG_IDENT)
