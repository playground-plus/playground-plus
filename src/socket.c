...
void lscripto(char *filename, char *string, int len)
{
  ...
  IGNORE_RET(write(fd, oldstack, len));
  close(fd);
  stack = oldstack;
 ...
 */
  ...
  {
    pg_log("boot", "Couldn't grab the socket!!!");
    exit(-1);
  }
  ...
  {
    pg_log("boot", "Couldn't bind socket - something is on that port number!");
    exit(-2);
  }
...
void do_alive_ping()
{
  ...
  if (!count && alive_descriptor > 0)
  {
	...
	return;
      ...
      pg_log("sigpipe", "Angel pipe closed");
...
void alive_connect()
{
  ...
  if (connect(alive_descriptor, (struct sockaddr *) &sa, sizeof(sa)) < 0)
  {
    ...
    alive_descriptor = -1;
    pg_log("boot", "Failed to connect to angel, we are on our own from here.");
    return;
  }
  do_alive_ping();

  pg_log("boot", "Alive and kicking");
...
void accept_new_connection(void)
{
  ...
  {
    pg_log("error", "EINTR accept trap");
    return;
  }
  if (new_socket < 0)
  {
  ...
  {
    IGNORE_RET(write(new_socket, full_msg.where, full_msg.length));
...
#else
  {
    ...
    if (getnameinfo((struct sockaddr *)&incoming, sizeof(incoming),
                    hostbuf, sizeof(hostbuf), NULL, 0, 0) == 0)
    {
      strncpy(p->inet_addr, hostbuf, MAX_INET_ADDR - 1);
      p->inet_addr[MAX_INET_ADDR - 1] = '\0';
    }
    else
    {
      strncpy(p->inet_addr, p->num_addr, MAX_INET_ADDR - 1);
      p->inet_addr[MAX_INET_ADDR - 1] = '\0';
    }
  }
#endif
  ...
  if (do_banish(p))
  {
    IGNORE_RET(write(new_socket, banish_msg.where, banish_msg.length));
  ...
  {
    IGNORE_RET(write(new_socket, splat_msg.where, splat_msg.length));
    out_current += banish_msg.length;
    out_pack_current++;
    destroy_player(p);
    return;
...
void get_player_input(player * p)
{
  ...
  {
    quit(p, 0);
    pg_log("error", "PANIC on FIONREAD ioctl");
  ...
  if (!chars_ready)
  {
	...
	pg_log("connection", "Connection went netdead on login.");
    }
    quit(p, 0);
    stack = oldstack;
    return;
...

  {
    char readbuf[4096];
    int nread, i;
    nread = read(p->fd, readbuf, chars_ready < (int)sizeof(readbuf) ? chars_ready : (int)sizeof(readbuf));
    if (nread <= 0)
    {
      pg_log("connection", "Caught read error on socket.");
      return;
    }
    for (i = 0; !(p->flags & PANIC) && i < nread; i++)
    {
      c = readbuf[i];
      switch (c)
      {
	...
	default:
	  ...
	  if ((c > 31) && (p->ibuff_pointer < (IBUFFER_LENGTH - 3)))
	  {
	    ...
	    if ((!(p->flags & PASSWORD_MODE)) && (p->flags & DO_LOCAL_ECHO))
	    {
	      ...
	      {
		pg_log("error", "Echoing back to player.");
		quit(p, 0);
		return;
	      }
	      out_current++;
...
{
  player *scan;
  struct pollfd fds[max_players + 8];
  int nfds = 0;
  int main_idx, ident_idx = -1, intercom_idx = -1;
  int player_start;

  /* main listening socket */
  main_idx = nfds;
  fds[nfds].fd = main_descriptor;
...
#ifdef IDENT
  if (ident_idx >= 0 && (fds[ident_idx].revents & POLLIN))
    read_ident_reply();
#endif
...
#ifdef INTERCOM
  if (intercom_idx >= 0 && (fds[intercom_idx].revents & POLLIN))
...
void set_talker_addy(void)
{
  ...
  {
    LOGF("error", "getaddrinfo fails for %s, errno %d, %s\n",
  ...
  if ((ptr = get_config_msg("site_alias")) && *ptr)
    ...
    getnameinfo(res->ai_addr, res->ai_addrlen,
                talker_alpha, 160, NULL, 0, 0);

  getnameinfo(res->ai_addr, res->ai_addrlen,
              talker_ip, 40, NULL, 0, NI_NUMERICHOST);
  freeaddrinfo(res);
}
...
void ping(player * p, char *str)
{
  ...
  *stack++ = '\0';
  ...
  IGNORE_RET(write(p->fd, oldstack, strlen(oldstack)));
  stack = oldstack;
...
void do_ping(player * p, char *str)
{
  ...
  for (scan = flatlist_start; count; scan = scan->flat_next)
    if (!scan)
    {
      ...
      pg_log("error", "Bad do_ping list (socket.c)");
      stack = oldstack;
      return;
    }
    else if (scan->name[0])
